...
import {
  ...
  Pill, UserSearch, LogOut, MapPin, Phone, Loader2, Map as MapIcon
} from 'lucide-react';
          ...
          <div>
            ...
            <nav className="space-y-1">
              {navItems.map(({ label, icon: Icon, path, id }) => (
                <button key={id} onClick={() => navigate(path)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-sm transition-colors ${id === 'doctor' ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-50'}`}>
                  <Icon className={`w-4 h-4 ${id === 'doctor' ? 'text-sky-500' : ''}`} />{label}
                </button>
              ))}
            </nav>
          </div>
